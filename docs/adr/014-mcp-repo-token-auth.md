...
- A new `mcp_tokens` table: `repo_id` (FK), a hash of the token secret (never the raw value), `created_at`, `last_used_at`, `revoked_at`. Only one active token per repo at a time; generating a new one revokes the previous one, so rotation is just "generate again."
