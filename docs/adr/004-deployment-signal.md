...
- **Manual dashboard toggle only** — simplest, fully accurate to intent, but the user explicitly wanted the tag-based default plus an MCP path, so this was folded in as a _result_ of the MCP tool rather than a separate UI-only mechanism.
