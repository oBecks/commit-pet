...
- Under **Advanced** in the App's settings, click **Make public** — a private App can only be installed on the owning user/org account (org owners and App managers can still install an org-owned App), which blocks anyone else from trying commit-pet on their own repo (see [ADR-009](adr/009-public-multi-tenant.md)).
