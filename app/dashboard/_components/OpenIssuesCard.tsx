  ...
  return (
    <div className="flex flex-col gap-3 rounded-2xl border border-dash-border bg-dash-card p-6">
      ...
      {pet.openIssueCount === 0 ? (
        <p className="text-sm text-dash-muted">No open issues — this pet is happy.</p>
      ) : (
        <>
          <p className="text-sm text-dash-heading">
            <span className="font-semibold text-sick-text">{pet.openIssueCount}</span> open issue
          ...
          <p className="text-xs text-dash-muted">Health recovers as these close — no action needed here.</p>
        </>
      )}
    </div>
  );
